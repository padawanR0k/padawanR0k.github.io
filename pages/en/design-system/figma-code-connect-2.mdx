...
## author: [padawanr0k] title: Figma Code Connect Tips date: 2024-12-27 tags: ["code-connect"] layout: post image: ../../../img/figma-code-connect.png keywords: ["figma", "code-connect"]

Still adding more...

A helpful official Figma repository to check out: https://github.com/figma/sds/blob/main/figma.config.json

### Use AI for prop mapping

If you're using an enterprise membership that includes Figma's AI plan, you can use the **[AI for prop mapping](https://www.google.com/search?q=https://www.figma.com/code-connect-docs/quickstart-guide/%23publish-code-connect-files:~:text%3DUse%2520AI%2520for%2520prop%2520mapping)** feature when you run `npx figma connect`.

### `importPaths`

You can change the import paths that appear in dev mode when you run `figma connect publish`.

By default, the code shows a hardcoded path from the `Button.figma.tsx` file:
...
By applying a setting in your `figma.config.json` file, you can change this:
...
This will change the import path to:
...
This is useful for overriding paths when `importPaths` doesn't give you the desired result.
...
Reusable nodes in Figma have a URL. Hardcoding these URLs into your code can cause problems if the nodes change in the design. This feature allows you to treat Figma node URLs as variables to avoid this issue.

```json
"documentUrlSubstitutions": {
  "<FIGMA_ICONS_BASE>": "[https://figma.com/design/J0KLPKXiONDRssXD1AX9Oi](https://figma.com/design/J0KLPKXiONDRssXD1AX9Oi)",
}
```
